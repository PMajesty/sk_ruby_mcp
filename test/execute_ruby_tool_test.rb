...
class ExecuteRubyToolTest < Minitest::Test
  ...
  def test_spec_describes_a_destructive_open_world_tool_with_code_required
    ...
    assert_includes spec[:outputSchema][:required], 'return_value'
    assert_includes spec[:description], 'undo'
  end

  def test_missing_code_is_a_tool_error_with_full_structured_content
    result = @tool.call({})
    assert result[:isError]
    assert_equal 'ArgumentError', result[:structuredContent][:error][:class]
    assert_equal ExecuteRuby::OUTPUT_SCHEMA[:required].sort, result[:structuredContent].keys.map(&:to_s).sort
  ...
  def test_defaults_wrap_in_operation_and_operation_name
    @tool.call('code' => '1 + 1')
    assert_equal [['1 + 1', 'MCP execute_ruby', true]], @executor.calls
  end

  def test_explicit_arguments_are_passed_through
    @tool.call('code' => 'x', 'operation_name' => "  Build roof  ", 'wrap_in_operation' => false)
    assert_equal [['x', 'Build roof', false]], @executor.calls
  end
  ...
  def test_default_wrap_can_be_disabled_by_configuration
    ...
    tool.call('code' => 'x')
    assert_equal false, @executor.calls.first.last
  end
  ...
  def test_successful_result_renders_text_and_structured_content
    ...
    assert_equal '2', result[:structuredContent][:return_value]
  end
